...
template <typename T>
void            test_cons(ft::list<T> lst)
{
    typename ft::list<T>::iterator beg(lst.begin());
    typename ft::list<T>::iterator end(lst.end());
    ft::list<T> new_lst(beg, end);

    test(new_lst);
}

template <typename T>
ft::list<T>        create_list(int size, const T& value1, const T &value2, const T &value3)
{
    ft::list<T>     lst(size, value1);
