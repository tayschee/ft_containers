...
template <typename T>
void    cmp(typename std::list<T> &lst1, typename std::list<T> &lst2)
{
    std::cout << (lst1 == lst2) << " ";
    std::cout << (lst2 == lst1) << " ";
    std::cout << (lst1 != lst2) << " ";
...
template <typename T>
std::list<T> create(size_t i, T val)
{
    return (std::list<T>(i, val));
}
...
void    test_all(T val, T val2, T *tab, size_t size, T *tab1, size_t size1)
{
    ...
    typename std::vector<T>::iterator beg1(tab1);
    typename std::vector<T>::iterator end1(&tab1[size1]);
    std::list<T> lst1;
    std::list<T> lst2;
    std::list<T> lst3(beg, end);
    std::list<T> lst4(lst3);
    std::list<T> lst5(beg1, end1);
    size_t i = 0;
    size_t j = 0;
