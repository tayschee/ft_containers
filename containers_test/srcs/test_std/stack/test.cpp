...
void    test(const std::stack<T, C> &s)
{
    std::cout << s.size();
    std::cout << s.empty();
}
...
template<class T, class C>
void    cmp(const std::stack<T, C> &s1, const std::stack<T, C> &s2)
{
    std::cout << (s1 == s2) << " ";
    std::cout << (s2 == s1) << " ";
    std::cout << (s1 != s2) << " ";
...
template<class T, class C>
void    pop(std::stack<T, C> &s, size_t size)
{
    size_t i = 0;
    while (i < size)
    {
...
template<class T, class C>
void    push(std::stack<T, C> &s, T *tab, size_t size)
{
    size_t i = 0;
    while (i < size)
    {
...
template<class T, class C>
void    test_all(std::stack<T, C> s, size_t del1, T *tab, size_t size, size_t del2)
{
    test(s);
    pop(s, del1);
    test(s);
...
int main()
{
    ...
    std::stack<int, std::list<int> >      si1;
    std::stack<int, std::list<int> >      si2(lst);

    std::string         s1("abc"), s2("uvk"), s3("j'en ai marre"), s4("pffffff..."), s5("8");
    std::string         tabs[] = {s1, s2, s3, s4, s5};
    std::vector<std::string>       vec(tabs, tabs + 5);
    std::stack<std::string, std::vector<std::string> >      ss1;
    std::stack<std::string, std::vector<std::string> >      ss2(vec);
    ...
    std::stack<char, std::list<char> >      sc1(lstc1);
    std::stack<char, std::list<char> >      sc2(lstc2);
