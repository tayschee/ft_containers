...
int main()
{
    ...
    for (i = 0; tab1[i] < 0; ++i);
    std::vector<float>::iterator beg1(tab1);
    std::vector<float>::iterator end1(&tab1[i]);
    std::vector<float>    vec1(beg1, end1);

    float    tab2[] = {9.4f, -0.4f};
    for (i = 0; tab2[i] < 0; ++i);
    std::vector<float>::iterator beg2(tab2);
    std::vector<float>::iterator end2(&tab2[i]);
    std::vector<float>    vec2(beg2, end2);

    float    tab3[] = {-0.1f};
    for (i = 0; tab3[i] < 0; ++i);
    std::vector<float>::iterator beg3(tab3);
    std::vector<float>::iterator end3(&tab3[i]);
    std::vector<float>    vec3(beg3, end3);
    ...
    for (i = 0; tab4[i] != 0; ++i);
    std::vector<float>::const_iterator beg4(tab4);
    std::vector<float>::const_iterator end4(&tab4[i]);
    std::vector<float>    vec4(beg4, end4);

    float    tab5[] = {12, 35,124, 0};
    for (i = 0; tab5[i] != 0; ++i);
    std::vector<float>::const_iterator beg5(tab5);
    std::vector<float>::const_iterator end5(&tab5[i]);
    std::vector<float>    vec5(beg5, end5);

    std::vector<float>    vec6(12, 12);
